...
const loop = async cb => {
  ...
  if (myId === senatorId) {
    ...
    // blockchain
    const block = await blockchain.generateBlock(transactions)
    console.log(block)
  }
}
  return Promise.resolve()
...
const getSenatorList = async () => {
  const SlotNumber = slotTool.getSlotNumber()
...
const getMySenatorId = async () => {
  ...
  let id = _.findIndex(senatorList.senatorList, (o) => {return o.publicKey = publicKey})
  id = id === -1 ? 'tan90' : id + 1
  return id
}
const start = () => {
