...
const stateRootModel = require('./waiting').stateRootModel
...
Runner.prototype.addBlock = async function() {
  ...
  try {
    ...
    if (_.isNil(block.stateRoot)) {
      ...
      waitting.set('block.' + h, block.serialize())
      // broadcast(block)
    } else {
      ...
      broadcast.broadcast('block', data)
      waitting.set('block.' + h, block.block.serialize())
      stateRootModel['stateRoot.' + h] = block.block.stateRoot
...
Runner.prototype.loop = async function() {
  ...
  const myId = this.getMySenatorId(publicKey)
...
Runner.prototype.getMySenatorId = async function(publicKey) {
