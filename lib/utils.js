...
class Utils {
  ...
  generatorSignature (msg, secretKey) {
    const signature = nacl.sign.detached(this.msgBuffer(msg), this.encode(secretKey))
    return Buffer.from(signature).toString('hex')
  }
  verifySignature (msg, signature, publicKey) {
    return nacl.sign.detached.verify(this.msgBuffer(msg), this.encode(signature), this.encode(publicKey))
  }
