...
class Utils {
  ...
  sign(msg, privateKey) {
    msg = _.isBuffer(msg) ? msg : Buffer.from(mag, 'hex')
    return secp256k1.sign(msg, privateKey)
  }

  verifySignature(msg, signature, publicKey) {
    msg = _.isBuffer(msg) ? msg : Buffer.from(mag, 'hex')
    signature = _.isBuffer(signature) ? msg : Buffer.from(signature, 'hex')
    publicKey = _.isBuffer(publicKey)
      ? publicKey
      : Buffer.from(signature, 'hex')
    return secp256k1.verify(msg, signature, publicKey)
  }
