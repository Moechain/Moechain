 ...
 */
/**
 * NOTES：this package is from ethereumjs-blockchain,
 * Moechain change some code for convenience,
 * Moechain-chain should use mgpl, becaue ethereumjs-chain use mgpl,
 * thanks to the help of ethereumjs-chain
 */
...
const Block = require('../moechain-block')
...
Blockchain.prototype._putBlock = function(block, cb, isGenesis) {
  ...
  async.series(
    [verify, lookupParentBlock, rebuildInfo, cb => self._batchDbOps(dbOps, cb)],
    cb
  )
  ...
  function lookupParentBlock(next) {
    ...
    self.getDetails(block.header.parentHash, function(err, _parentDetails) {
      ...
      if (!err && parentDetails) {
        ...
        let parentHash = ethUtil.bufferToHex(
  ...
  function rebuildInfo(next) {
    ...
    if (block.isGenesis() || totalDifficulty.cmp(self.meta.td) === 1) {
      ...
      const blockNumber = parseInt(
        block.header.number.toString('hex') || '00',
        16
      ).toString()
...
Blockchain.prototype.getBlocks = function(
  ...
  function nextBlock(blockId) {
    self.getBlock(blockId, function(err, block) {
      ...
      var nextBlockNumber =
        ethUtil.bufferToInt(block.header.number) + (reverse ? -1 : 1)
...
Blockchain.prototype._rebuildBlockchain = function(
  hash,
  parentHash,
  parentDetails,
  ops,
  ...
  async.series([loadNumberIndex, loadStaleDetails, getNextDetails], function(
    err
  ) {
    if (err) return cb(err)
    self._rebuildBlockchain(
...
Blockchain.prototype.delBlock = function(blockhash, cb) {
  ...
  async.series(
    [buildDBops, getLastDeletesDetils, cb => self._batchDbOps(dbOps, cb)],
    cb
  )
...
Blockchain.prototype._delBlock = function(blockhash, dbOps, resetHeads, cb) {
  ...
  async.series([getDetails, removeChild, removeStaleChildren], cb)
...
Blockchain.prototype._iterator = function(name, func, cb) {
  ...
  function run(cb2) {
    ...
    async.series([getDetails, getBlock, runFunc, saveDetails], function(err) {
      if (!err) {
