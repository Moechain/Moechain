...
/**
 * NOTES：this package is from ethereumjs-block,
 * Moechain change some code for convenience,
 * Moechain-block should use mgpl, becaue ethereumjs-block use mgpl,
 * thanks to the help of ethereumjs-block
 */
/**
...
Block.prototype.validateTransactionsTrie = function() {
  ...
  // console.log(txT)
  // console.log(this.txTrie.root.toString('hex'))
...
Block.prototype.validateTransactions = function(stringError) {
  ...
  this.transactions.forEach(function(tx, i) {
    var error = new Tx(tx).validate(true)
    if (error) {
...
Block.prototype.toJSON = function(labeled) {
  if (labeled) {
    ...
    this.transactions.forEach(function(tx) {
      obj.transactions.push(new Tx(tx).toJSON(labeled))
    })
