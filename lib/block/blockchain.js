...
const peers = require('../network/peerCache').cache
...
exports.ping = async url => {
  ...
  return new Promise((resolve, reject) => {
    client.get('posts/', function(err, res, body) {
      return resolve(body.rows[0].blockHeight)
    })
  })
}

exports.getHeight = async received => {
  try {
    const random = _.random(0, received.length)
    const peer = received[random]
    const peerUrl = 'http://' + peer.host + ':' + peer.port + '/api/peer/'
    let height = await exports.ping(peerUrl)
...
exports.getBlocksFromPeer = async (url, start, end) => {
  const client = request.createClient(url)
  url = url + '/api/block'
  client.get('blocks/?start=' + start + '&end=' + end, function(
    err,
    res,
    body
  ) {
    return resolve(body.rows[0].blocks)
  })
}

//i should improve and perfect this function

exports.loadblocks = async () => {
  try {
    const block = await exports.getHead()
    const blockNumber = new ethUtil.BN(block.header.number).toNumber()
    if (blockNumber === 0) {
      let received = []
      peers.forEach((value, key, cache) => {
        if (value.state === 2) {
          received[key] = value
        }
      })
      if (received.length !== 0) {
        let { height, url } = await exports.getHeight(received)
        let start = 0
        let end = height
        let blocks = await exports.getBlocksFromPeer()
        blocks.forEach(item => {
...
exports.loadblocks().then(n => console.log(n))
