...
function genAccountTrie() {
    ...
    //     console.log(to.hasOwnProperty(item.to.toString('hex')))
    //     let value = new BN(item.value)
    //     let to_account = item.to.toString('hex')
    //     if (to.hasOwnProperty(to_account)) {
    //         to[to_account] = new BN(to[to_account]).add(value).toString('hex')
    //     } else {
    //         to[to_account] = value.toString('hex')
    //     }
    // })
    let em = []
    /*
    transactions.forEach((item, index) => {
        // console.log(from.hasOwnProperty(item.from.toString('hex')))
        let value = parseInt(item.value.toString('hex'))
        let from_account = item.from.toString('hex')
        if (from.hasOwnProperty(from_account)) {
            // from[from_account] = new BN(from[from_account]).add(new BN(value)).toString('hex')
            from[from_account] = from[from_account] + value
        } else {
        ...
        // if (from.hasOwnProperty(from_account)) {
        //     from[from_account] = new BN(from[from_account]).add(value).toString('hex')
        // } else {
        //     from[from_account] = value.toString('hex')
        // }
