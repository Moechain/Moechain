...
exports.genAccountTrie = function(txs, stateRoot, cb) {
  ...
  _.mapKeys(from, function(value, key) {
    if (_.has(to, key)) {
      let trueValue = new BI(to[key]).add(new BI(value)).toString()
      to[key] = trueValue
    }
    to[key] = value
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    _.forEach(account, function(value, key) {
...
exports.generateBlock = function(txs) {
  let blockchain = new Blockchain({ blockDb: blockDb, detailsDb: detailsDb })
  return exports.getHead().then(b => {
    return new Promise((resolve, reject) => {
      let block = new Block()
...
let w = async () => {
  try {
    let r = await exports.genAccountTrie(
      transactions,
      '0c7300a0f7a3c9f6e43297991878c48c5372b190a46254c0eddfc7b90b0fcd2b'
    )
