...
router.get('/block', async ctx => {
  ...
  const flag = _.toInteger(query.flag)
  const block = await blockchain.getBlock(flag)

  ctx.body = { block: block.serialize().toString() }
})
router.get('/blocks', async ctx => {
  const query = ctx.request.query
  const start = _.toInteger(query.start)
  const end = _.toInteger(query.end)
  let blocks = await blockchain.getBlocks(start, end)
  blocks = blocks.map(item => {
    return item.serialize().toString('hex')
  })
  // console.clear()
  ctx.body = { blocks }
})
